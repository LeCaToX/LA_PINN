...
for ig = 1:length(numGaussList)
    ...
    net = buildKAN(2,2,64,4,baseProb.a);
...
% buildNet keeps the original calling syntax, but constructs a KAN instead
% of an MLP. Each KAN layer first expands every scalar channel through a
% fixed cubic B-spline basis and then learns the edge coefficients using a
% fully connected linear map. No pointwise activation function is used.
%
% KAN layer: z_j = sum_i sum_m c_{jim} B_m(x_i) + b_j.

numKnots = 8;  % number of spline centres per scalar channel

layers = [
    featureInputLayer(inDim,'Normalization','none','Name','input')
    functionLayer(@(X) kanBasisExpansion(X,numKnots), ...
...
function Z = kanBasisExpansion(X,numKnots)
% Cubic B-spline feature map used by every KAN edge.
% X has size [numChannels x numPoints]. The output contains, for each input
% channel, a constant/base feature, a linear feature, and numKnots cubic
% B-spline features. Hidden states are smoothly mapped to [-1,1] so that
% the same fixed spline grid can be used at all KAN layers.

Xn = tanh(X);
centres = single(linspace(-1,1,numKnots));
h = single(2/(numKnots-1));

numChannels = size(Xn,1);
features = cell(numChannels*(numKnots+2),1);
row = 0;
...
for i = 1:numChannels
    ...
    for j = 1:numKnots
        t = abs((xi-centres(j))./h);
        row = row + 1;
        features{row} = cubicBSpline(t);
    end
end
...
% Centred cardinal cubic B-spline with compact support |t| < 2.
