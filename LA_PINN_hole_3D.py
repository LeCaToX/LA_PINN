...
prob.R = 0.4      # spherical hole radius
...
class MLP(nn.Module):
    def __init__(self, inDim, outDim, width, depth):
        ...
        layers = []
        layers.append(nn.Linear(inDim, width))
        ...
        layers.append(nn.Linear(width, outDim))
...
def buildNet(inDim, outDim, width, depth):
    return MLP(inDim, outDim, width, depth).to(device)
...
def plotDissipation3Views(prob, Dnode):
    ...
    for iv in range(3):
        ...
        ax.set_xlabel("x-$p_1$", fontname="Times New Roman", labelpad=-8)
        ax.set_ylabel("y-$p_3$", fontname="Times New Roman", labelpad=-8)
        ax.set_zlabel("z-$p_2$", fontname="Times New Roman", labelpad=-8)
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_zticks([])
        ax.grid(False)
...
def plotHistory(iterHist, lambdaHist, nAdam):
    ...
    plt.plot(iterHist[idAdam], lambdaHist[idAdam], "b-", linewidth=2)

    if np.any(~idAdam):
        plt.plot(iterHist[~idAdam], lambdaHist[~idAdam], "r-", linewidth=2)
        plt.axvline(nAdam, color="k", linestyle="--")
        plt.legend(["Adam", "LBFGS"], loc="best")
    else:
        plt.legend(["Adam"], loc="best")
